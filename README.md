...
Для работы с данным документом созданы предопределенные профили пользователей Специалист и Менеджер по обслуживанию
...
Для работы с данным документом создан предопределенный профиль пользователей Бухгалтер ИТ Фирмы.
...
Для работы с данной обработкой создана роль BKM_Массовое создание документов Реализации. Эта роль подключена к предопределенному профилю пользователей Главный бухгалтер.
...
Для работы с подсистемой Расчёт управленческой зарплаты создан предопределенный профиль пользователей Кадровик-расчетчик.
# Отчеты
...
# Все настройки осуществляются в режиме 1С предприятия:
  ...
  2. Создать пользователей с правами доступа Главный бухгалтер, Кадровик-расчетчик, Специалист, Менеджер по обслуживанию выбрав соответствующий профиль пользователя
# Подсистема Обслуживание клиентов.
  1. В режиме администратора настроить: [инструкция к настройке, нажмите на ссылку пожалуйста](https://github.com/ArkadiyKorobka/12_01_2026/blob/main/TELEGRAM.md)
  2. Настроить регламентное задание по отправке сообщений телеграмм-боту.
  3. Из полученных данных настройки Телеграмм-бота заполнить Константы Токен управленияТГ ботом например: 8332950524:AAGIqt26kq2_pvwEAU8SCXmQEFZ7J5WHdCI и Идентификатор группы для оповещения например: -5177352191 группа: https://t.me/+6tOOxKy9R50xYzhi
  4. Создать номенклатуру (или выбрать существующую) с видом номенклатуры Услуги, которая будет отображаться в актах об оказании услуг клиенту. Рекомендуется Абонентская плата и Работы специалиста. Заполнить созданными (выбранными) наименованиями соответствующие константы: НоменклатураАбонентскаяПлата и НоменклатураРаботыСпециалиста.
# Перед первым использованием подсистемы Обслуживание клиентов, Кадровику-расчетчику необходимо
1. Подсистема Обслуживание клиентов использует справочник BKM_Сотрудники. На данном этапе справочник пуст.
...
4. Первым идет запуск автотестирования из файла Создание_документов_Обслуживания_клиента.feature.
5. Затем - Закрытие_документа_Обслуживание_клеента.feature
6. Затем - Массовое_создание_актов.feature
7. Для последнего тестирования необходимо сохранить на компьютер файлы из репоитория Ноябрь.mxl и октябрь.mxl.
...
8. После запуска вышеуказанного пакета можно повторно запускать тесты в произвольном порядке.
